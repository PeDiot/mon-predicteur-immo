...
#setwd("C:/Users/pemma/OneDrive - GENES/Ensae/Business Data Challenge/business-data-challenge/code/exploration")
```
...
system.time(
  dat <- fread(input = paste0(DATA_DIR, params$file_name, ".csv"), 
               encoding = "UTF-8" 
               # ,na.strings = c("", NA)
               )
)
...
file_name <- paste0(BACKUP_DIR, "exploration/missing_", params$file_name, ".RData")
...
dat_cat <- dat_cat %>%
  mutate(type_local2 = dat$type_local2)
...
```{r}
make_pct_countplot(var_name = "type_local2")
```
...
```{r eval=FALSE}
table(dat$nature_culture, dat$code_nature_culture) %>% View()
```
...
n <- dat %>% nrow() %>% format(big.mark = " ", scientific = F)
n_bis <- dat_no_outliers %>% nrow() %>% format(big.mark = " ", scientific = F)
```

On décide de supprimer les valeurs aberrantes ainsi que les biens dont le nombre de pièces principales est supérieur à 12 : 
...
Afin de pouvoir construire des visualisations assez rapidement, on décide de travailler sur un sous-échantillon de la base de données de taille `r n_samples`. 
...
for (mask in cat_vars) {
  ...
  if (n_levels <= 15) {
    ...
    plot_list[[mask]] <- histogram_per_category(df = df_subset,
                                                cat_var = mask, 
                                                target = target) 
